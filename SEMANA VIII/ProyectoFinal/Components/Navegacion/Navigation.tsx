      ...
      <PaperProvider>
        <NavigationContainer>
          <Tab.Navigator initialRouteName="Inicio" screenOptions={{ tabBarStyle: { backgroundColor: '#0d6efd', borderTopWidth: 0, }, tabBarActiveTintColor: '#fff', tabBarInactiveTintColor: 'rgba(255, 255, 255, 0.6)', }} >
            ...
            <Tab.Screen name='Tareas' component={Task} options={{ tabBarIcon: ({ color, size }) => (<Ionicons name="desktop" color={color} size={size} />), }}></Tab.Screen>
            <Tab.Screen name='Indicadores' component={Home} options={{ tabBarIcon: ({ color, size }) => (<Ionicons name="bar-chart" color={color} size={size} />), }}></Tab.Screen>
          </Tab.Navigator>
        </NavigationContainer>
      </PaperProvider>
